...
# UI 모듈
st_news_q_ui <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(
      column(8, selectInput(ns("select_pdf"), "설문지 선택:", choices = NULL)),
      br(),
...
# 서버 모듈
st_news_q_server <- function(input, output, session) {
  pdf_files_path <- system.file("st_news/pdf", package = "bitJowoncni")
  shiny::addResourcePath("pdfs", pdf_files_path)

  pdf_files <- list.files(path = pdf_files_path, full.names = TRUE, pattern = "q_*_*")
